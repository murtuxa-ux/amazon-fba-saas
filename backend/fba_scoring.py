"""
FBA Viability Scoring Engine — Ecom Era Wholesale
Scores products 0-100 across 5 dimensions.
"""
...
def score_sales_velocity(monthly_sales):
...
def score_price_stability(volatility_pct):
...
def score_competition(fba_sellers):
...
def compute_fba_score(bsr, monthly_sales, price_volatility_pct, fba_sellers, current_price):
    ...
    if total >= 80:   verdict = "Winner ✅"
    elif total >= 60: verdict = "Maybe 🟡"
    else:             verdict = "Skip ❌"
    return {
        "fba_score": total,
        "verdict": verdict,
...
def compute_profit(price, cost, referral_pct=0.15, fba_fee=3.22, other_costs=0.0):
