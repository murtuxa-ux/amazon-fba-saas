...
# Email SMTP Configuration — read directly from env vars (config.py uses extra="ignore")
SMTP_CONFIG = {
...
if not SMTP_CONFIGURED:
    logger.warning(
        ...
        "Set SMTP_USER and SMTP_PASSWORD in environment to enable email sending."
    )
...
def send_email(
    ...
    try:
        ...
        # Send via SMTP — force IPv4 (Railway containers lack IPv6 connectivity)
        ...
        with smtplib.SMTP(ipv4_addr, port, timeout=15) as server:
            server.ehlo(host)
            server.starttls()
            server.ehlo(host)
