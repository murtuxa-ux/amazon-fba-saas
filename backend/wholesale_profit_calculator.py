...
async def get_analysis_history(
    ...
    try:
        ...
        # Limit results
        query = query.order_by(ProfitAnalysis.created_at.desc()).limit(filters.limit).all()
...
async def health_check():
    ...
    return {"status": "healthy", "service": "profit-calculator"}nit * STORAGE_FEES["peak_month"]
...
async def health_check():
    """Health check endpoint"""
    return {"status": "healthy", "service": "profit-calculator"}fba_roi <= filters.max_roi)
        if filters.start_date:
            query = query.filter(ProfitAnalysis.created_at >= filters.start_date)
        if filters.end_date:
            query = query.filter(ProfitAnalysis.created_at <= filters.end_date)
