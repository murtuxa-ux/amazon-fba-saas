...
def forgot_password(data: ForgotPasswordInput, db: Session = Depends(get_db)):
    """Send password reset email (or show success regardless for security)"""
    email = data.email.strip().lower()
    user = db.query(User).filter(User.email == email).first()
    # Always return success to prevent email enumeration
    if user:
        # TODO: integrate email service (SendGrid/SES) for actual reset link
        _log.info(f"Password reset requested for user: {user.username} ({email})")
    return {
