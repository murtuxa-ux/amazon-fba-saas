...
def on_startup():
    ...
    # Add missing columns to existing tables (create_all does not alter existing tables)
    try:
        from sqlalchemy import text as _text
        with engine.connect() as _conn:
            _conn.execute(_text("ALTER TABLE users ADD COLUMN IF NOT EXISTS last_login TIMESTAMP"))
            _conn.commit()
            _log.info("Column migration completed successfully")
    except Exception as _me:
        _log.warning(f"Column migration skipped: {_me}")
