...
# Amazon API Integration & PPC Action Plan
_amazon_router = None
_ppc_action_plan_router = None
try:
    from amazon_integration import router as _amazon_router
except Exception as _e:
    _log_msg = f"Amazon integration not loaded: {_e}"

try:
    from ppc_action_plan import router as _ppc_action_plan_router
except Exception as _e:
    _log_msg2 = f"PPC Action Plan not loaded: {_e}"
...
for _mod_name, _router_name, _key in [
    ...
    ("client_portal", "router", "client_portal_legacy"),
    ("intelligence_hub", "router", "intelligence"),
    # Phase 2-10 modules
    ("client_portal", "router", "client_portal_v2"),
    ("inventory_restock", "router", "inventory_restock"),
    ("ppc_advanced", "router", "ppc_advanced"),
    ("wholesale_hub", "router", "wholesale_hub"),
    ("private_label", "router", "private_label"),
    ("finance_pl", "router", "finance_pl"),
    ("reporting_engine", "router", "reporting_engine"),
    ("team_workflow", "router", "team_workflow"),
    ("ai_automation", "router", "ai_automation"),
]:
    try:
...
# Amazon Integration & PPC Action Plan routers
if _amazon_router:
    app.include_router(_amazon_router)
if _ppc_action_plan_router:
    app.include_router(_ppc_action_plan_router)
...
def on_startup():
    try:
        ...
        _log.error(f"create_all error (non-fatal): {_create_err}")


# ── Health Check ────────────────────────────────────────────────────────────────
@app.get("/health")
def health_check():
    return {"status": "ok", "version": "8.0", "service": "Ecom Era FBA SaaS"}
