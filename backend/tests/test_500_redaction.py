...
Tests force a 500 by hitting a route we register dynamically inside
the test app — main.py doesn't have a "/boom" route by design, so we
use FastAPI's `add_api_route` to plug one in for the duration of the
test, then strip it after.
"""
...
@pytest.fixture
def _boom_route():
    """Register a route that raises ValueError so the global handler fires."""
    ...
    try:
        ...
        # Strip the test route so subsequent tests don't see it.
        app.router.routes = [
...
def test_dev_500_includes_exception_type_and_path(client, _boom_route, monkeypatch):
...
def test_prod_500_redacts_exception_type_and_path(client, _boom_route, monkeypatch):
...
def test_railway_env_var_alone_triggers_redaction(client, _boom_route, monkeypatch):
