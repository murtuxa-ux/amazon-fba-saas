              ...
              <div style={{ ...cardStyle, marginTop: '1rem' }}>
                ...
                {lookupData.score_breakdown && (
                  <div style={{ marginBottom: '16px' }}>
                    ...
                    {(() => {
                      const sb = lookupData.score_breakdown;
                      const v = (dim) => typeof dim === 'object' && dim !== null ? dim.score : (dim || 0);
                      const m = (dim, fallback) => typeof dim === 'object' && dim !== null ? dim.max : fallback;
                      return (
                        <>
                          <DimensionBar label="BSR Rank" value={v(sb.bsr)} max={m(sb.bsr, 30)} />
                          <DimensionBar label="Sales Velocity" value={v(sb.sales_velocity)} max={m(sb.sales_velocity, 20)} />
                          <DimensionBar label="Price Stability" value={v(sb.price_stability)} max={m(sb.price_stability, 20)} />
                          <DimensionBar label="Competition" value={v(sb.competition)} max={m(sb.competition, 20)} />
                          <DimensionBar label="Price Point" value={v(sb.price_point)} max={m(sb.price_point, 10)} />
                        </>
                      );
                    })()}
                  </div>
