            ...
            {lookupData && (
              <div style={{ ...cardStyle, marginTop: '1rem' }}>
                ...
                <div style={{
                  ...
                  {[
                    { label: 'BSR', value: lookupData.bsr != null ? lookupData.bsr.toLocaleString() : 'N/A', color: '#FFF' },
                    { label: 'Monthly Sales', value: lookupData.monthly_sales != null ? lookupData.monthly_sales.toLocaleString() : 'N/A', color: '#10B981' },
                    { label: 'Current Price', value: lookupData.current_price != null ? `$${lookupData.current_price.toFixed(2)}` : 'N/A', color: '#FFF' },
                    { label: 'FBA Sellers', value: lookupData.fba_sellers != null ? lookupData.fba_sellers : 'N/A', color: lookupData.fba_sellers <= 3 ? '#10B981' : lookupData.fba_sellers <= 6 ? '#FFD700' : '#EF4444' },
                    { label: 'Total Sellers', value: lookupData.total_sellers != null ? lookupData.total_sellers : 'N/A', color: '#FFF' },
                    { label: 'Reviews', value: lookupData.reviews != null ? lookupData.reviews.toLocaleString() : 'N/A', color: '#FFF' },
                    { label: 'Rating', value: lookupData.rating != null ? `${lookupData.rating} ★` : 'N/A', color: '#FFD700' },
                    { label: 'Price Volatility', value: lookupData.price_volatility_pct != null ? `${lookupData.price_volatility_pct.toFixed(1)}%` : 'N/A', color: lookupData.price_volatility_pct <= 10 ? '#10B981' : '#EF4444' },
                  ].map((s, i) => (
                ...
                <div style={{
                  ...
                  <div style={{ background: '#0A0A0A', borderRadius: '8px', padding: '12px', textAlign: 'center' }}>
                    ...
                    <span style={{ color: '#FFF', fontSize: '16px', fontWeight: 700 }}>
                      {lookupData.avg_price_90d != null ? `$${lookupData.avg_price_90d.toFixed(2)}` : 'N/A'}
                    </span>
                  </div>
                  <div style={{ background: '#0A0A0A', borderRadius: '8px', padding: '12px', textAlign: 'center' }}>
                    ...
                    <span style={{ color: '#EF4444', fontSize: '16px', fontWeight: 700 }}>
                      {lookupData.min_price_90d != null ? `$${lookupData.min_price_90d.toFixed(2)}` : 'N/A'}
                    </span>
                  </div>
                  <div style={{ background: '#0A0A0A', borderRadius: '8px', padding: '12px', textAlign: 'center' }}>
                    ...
                    <span style={{ color: '#10B981', fontSize: '16px', fontWeight: 700 }}>
                      {lookupData.max_price_90d != null ? `$${lookupData.max_price_90d.toFixed(2)}` : 'N/A'}
                    </span>
                  </div>
                </div>
                ...
                <div style={{
                  ...
                  {lookupData.net_profit != null && (
                    <div style={{
                      display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '10px', marginTop: '12px'
                    }}>
                      {[
                        { label: 'Referral Fee', val: `$${(lookupData.referral_fee || 0).toFixed(2)}`, c: '#EF4444' },
                        { label: 'Total Fees', val: `$${(lookupData.total_fees || 0).toFixed(2)}`, c: '#EF4444' },
                        { label: 'Net Profit', val: `$${(lookupData.net_profit || 0).toFixed(2)}`, c: lookupData.net_profit > 0 ? '#10B981' : '#EF4444' },
                        { label: 'ROI', val: `${(lookupData.roi_pct || 0).toFixed(1)}%`, c: lookupData.roi_pct >= 30 ? '#10B981' : lookupData.roi_pct >= 15 ? '#FFD700' : '#EF4444' },
                      ].map((m, i) => (
                        ...
                        {bulkResults.results.map((r, i) => (
                          <tr key={i} style={{ borderBottom: '1px solid #1E1E1E' }}>
                            ...
                            <td style={{ textAlign: 'right', padding: '10px' }}>
                              ${r.current_price != null ? r.current_price.toFixed(2) : 'N/A'}
                            </td>
                            <td style={{ textAlign: 'right', padding: '10px', color: '#10B981' }}>
                              {(r.monthly_sales || 0).toLocaleString()}
                            </td>
                            <td style={{ textAlign: 'right', padding: '10px', color: getScoreColor(r.fba_score), fontWeight: 700 }}>
                              {r.fba_score != null ? r.fba_score : '-'}
                            </td>
                  ...
                  <tbody>
                    {history.map((item, idx) => (
                      <tr key={idx} style={{ borderBottom: '1px solid #1E1E1E' }}>
                        ...
                        <td style={{ textAlign: 'right', padding: '10px', color: '#10B981' }}>
                          {item.monthly_sales != null ? item.monthly_sales.toLocaleString() : '-'}
                        </td>
                        <td style={{ textAlign: 'right', padding: '10px' }}>
                          ${item.current_price != null ? item.current_price.toFixed(2) : 'N/A'}
                        </td>
                        <td style={{ textAlign: 'right', padding: '10px', color: getScoreColor(item.fba_score), fontWeight: 700 }}>
                          {item.fba_score != null ? item.fba_score : '-'}
                        </td>
                        <td style={{ textAlign: 'center', padding: '10px' }}>
                          <span style={{
