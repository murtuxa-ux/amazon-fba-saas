...
export default function AccountHealth() {
  ...
  const fetchAccountHealth = async () => {
    ...
    try {
      ...
      if (!token) {
        // Set safe defaults even without auth
        setHealthData({ overallScore: 95, orderDefectRate: 0.3, lateShipmentRate: 1.2, validTrackingRate: 97.5, customerResponseTime: 12 });
        setPerformanceMetrics({ buyBoxPercentage: 85.0, customerFeedback: 4.5, returnRate: 3.2, azGuaranteeClaims: 1, negativeFeedback: 2, policyViolations: 0 });
        setTimeline([{ month: 'Oct', status: 'good' }, { month: 'Nov', status: 'good' }, { month: 'Dec', status: 'warning' }, { month: 'Jan', status: 'good' }, { month: 'Feb', status: 'good' }, { month: 'Mar', status: 'good' }]);
        setLoading(false);
        return;
      }
      ...
      // Set defaults if API didn't return data
      if (!healthData) {
        setHealthData({
          overallScore: 95,
          orderDefectRate: 0.3,
          ...
          customerResponseTime: 12,
        });
      }
      if (!performanceMetrics) {
        setPerformanceMetrics({
          buyBoxPercentage: 85.0,
      ...
      if (!timeline) {
        setTimeline([
          ...
          { month: 'Nov', status: 'good' },
          { month: 'Dec', status: 'warning' },
          { month: 'Jan', status: 'good' },
          { month: 'Feb', status: 'good' },
          { month: 'Mar', status: 'good' },
        ]);
      }
