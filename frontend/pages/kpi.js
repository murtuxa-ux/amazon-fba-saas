...
export default function KPIPage() {
  ...
  const getPeriodLabel = () => {
    if (period === "monthly") return `${MONTHS[targetMonth - 1]} ${targetYear}`;
    if (period === "quarterly") return `Q${targetQuarter} ${targetYear}`;
    if (period === "yearly") return `${targetYear}`;
    return `Week ${getISOWeek()} ${targetYear}`;
  };
  ...
  useEffect(() => {
    ...
    fetchKPIData();
    fetchTargets();
  }, [period, targetMonth, targetQuarter, targetYear]);
  ...
  const fetchTargets = async () => {
    try {
      ...
      const res = await fetch(`${API_URL}/kpi/targets?period=${period}&period_label=${encodeURIComponent(pl)}`, {
        headers: authHeader(),
      });
      if (res.ok) {
        const data = await res.json();
                  ...
                  <>
                    ...
                    <div style={{
                      ...
                      <div style={{ minWidth: "130px" }}>
                        <label style={{ display: "block", fontSize: "11px", fontWeight: "600", color: textSec, textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: "6px" }}>
                          Period Type
                        </label>
                      ...
                      {period === "quarterly" && (
                        <div style={{ minWidth: "100px" }}>
                          ...
                          <select value={targetQuarter} onChange={e => setTargetQuarter(Number(e.target.value))}
                            style={{ ...inputStyle, cursor: "pointer" }}>
